test_that("yeahr_rename handles duplicates in rename_vector", {
  input_dir <- tempdir()
  output_dir <- file.path(tempdir(), "output")
  rename_vector <- c("file1" = "new_file1", "file1" = "new_file2")

  expect_error(
    yeahr_rename(input_dir, output_dir, rename_vector),
    "The names in the rename vector contain duplicates."
  )
})

test_that("yeahr_rename renames files correctly", {
...
test_that("yeahr_rename skips files with no matches", {
  input_dir <- file.path(tempdir(), "input")
  output_dir <- file.path(tempdir(), "output")
  dir_create(input_dir)
  dir_create(output_dir)
...
test_that("yeahr_rename skips files with multiple matches", {
  input_dir <- file.path(tempdir(), "input")
  output_dir <- file.path(tempdir(), "output")
  dir_create(input_dir)
  dir_create(output_dir)
